...
import { getComponentFiles, getLayoutTemplate, getOriginalRoutePath, getPageFiles, getRoutePath, saveClientComponentModule, saveClientRoutesFile, saveComponentHtmlDisk, saveServerRoutesFile, readFile, getImportData, generateComponentId, calculateRelativePath, CLIENT_SERVICES_DIR, CLIENT_COMPONENTS_DIR, adjustClientModulePath } from "./files.js";
...
export async function generateComponentsAndFillCache() {
  ...
  const [pageFiles, componentFiles] = await Promise.all([
    getPageFiles(),
    getComponentFiles() // todo, in future we will use imports from pages to detect used components only
  ]);
