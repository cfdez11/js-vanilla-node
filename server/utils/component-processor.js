...
import { getComponentFiles, getLayoutTemplate, getOriginalRoutePath, getPageFiles, getRoutePath, saveClientComponentModule, saveClientRoutesFile, saveComponentHtmlDisk, saveServerRoutesFile, readFile, getImportData, getAutogeneratedComponentName } from "./files.js";
...
function generateClientScriptTags({
  ...
  const scripts = `
    ${addHydrateClientComponentsScript
      ? `<script src="/public/_app/services/hydrate-client-components.js" type="module"></script>`
      : ""
    }
 ...
 * Absolute path to the HTML component file.
 *
...
export async function generateClientComponentModule(componentPath) {
  ...
  const componentMetadata = getMetadata
  ...
  const clientComponentModule = `
    ...
    export const metadata = ${JSON.stringify(componentMetadata)}
...
async function generateComponentAndFillCache(filePath) {
  ...
  const serverHtmls = await generateServerComponentHTML(filePath);
    ...
    if (serverHtmls.length) {
      ...
      return 'Server component generated';
    }

    const jsModuleCode = await generateClientComponentModule(filePath);
...
async function getRouteFileData(file) {
  ...
  const { getData, getMetadata, clientCode } = await processHtmlFile(file.fullpath);
  ...
  data.serverRoute = `{
    ...
    meta: {
      ssr: "${isSSR}",
      requiresAuth: false,
      revalidateSeconds: ${metadataPage?.revalidateSeconds} ?? 0,
    },
  }`;
